...
if(dist_raw < 180.0) {
  ...
  else if(dist_raw < 360.0 && dist_raw > 180.0){
    pinMode(PIN_LED,OUTPUT);
    float alpha = 0.9;
    dist_ema = (alpha)*(dist_raw) + (1-alpha)*(dist_ema);
    myservo.writeMicroseconds(int(_DUTY_NEU -10 *(270 - dist_ema)));
  }
  else {
    pinMode(PIN_LED,INPUT);
    myservo.writeMicroseconds(_DUTY_MAX+100);
  }
